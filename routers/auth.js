...
router.post("/create", accountCreationLimit, async (req, res) => {
    ...
    if (config.authentication.turnstile_secret_key) {
        if (!turnstileToken) {
        ...
        try {
            const verifyResponse = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
                ...
                body: `secret=${encodeURIComponent(config.authentication.turnstile_secret_key)}&response=${encodeURIComponent(turnstileToken)}&remoteip=${encodeURIComponent(req.ip)}`
            });
